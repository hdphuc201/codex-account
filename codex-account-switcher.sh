...
CODEX_ACCOUNT_SWITCHER_DIR="${CODEX_ACCOUNT_SWITCHER_DIR:-$CODEX_HOME/.codex-account-switcher}"
ACCOUNTS_HOME="${CODEX_ACCOUNTS_HOME:-$CODEX_ACCOUNT_SWITCHER_DIR/accounts}"
BACKUPS_HOME="${CODEX_BACKUPS_HOME:-$ACCOUNTS_HOME}"
STATE_FILE="${CODEX_CURRENT_ACCOUNT_FILE:-$CODEX_ACCOUNT_SWITCHER_DIR/.current-account}"
...
Notes:
  ...
  - Saved accounts live in ~/.codex/.codex-account-switcher/accounts.
...
ensure_dirs() {
  mkdir -p "$CODEX_HOME" "$CODEX_ACCOUNT_SWITCHER_DIR" "$ACCOUNTS_HOME"
  chmod 700 "$CODEX_ACCOUNT_SWITCHER_DIR" 2> /dev/null || true
